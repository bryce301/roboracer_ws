...
三圈数据表明，紧弯处最终速度确实主要受转角限速约束，但这正是避免第一版高速入弯失效的安全边界。当前直接角度映射已经在 15 Hz 下稳定完成约 33 秒圈速。

如果直接使用完整目标距离计算 Pure Pursuit 曲率，5–6 m 的远目标会产生很小的转角，可能导致车辆在急弯中转向过晚。因此在没有系统确定前视距离之前，不应直接替换当前转向映射。

### 6.4 后续实施方案

曲率功能应作为独立、默认关闭的实验参数实现，不能直接覆盖已验证控制器。

建议步骤：

1. 添加 `use_curvature_steering` 参数，默认 `False`；
2. 添加 `curvature_lookahead` 参数，初始范围建议 0.55–0.80 m；
